...
function normalizeMarkdownBody(content, options = {}) {
	...
	const rewrittenHtmlAttributes = rewriteHtmlAttributes(
		rewrittenMarkdownLinks,
		assetRewriter,
	);
	const normalizedBranding = normalizeBranding(rewrittenHtmlAttributes);
	const normalizedOrderedLists =
		normalizeOrderedListMarkers(normalizedBranding);
	return normalizeTrailingWhitespace(normalizedOrderedLists);
}
...
function normalizeBranding(text) {
	...
	return text.replace(/\bGithub\b/g, "GitHub");
}
...
function normalizeTrailingWhitespace(text) {
	...
	const withoutTrailingBlankLines = normalizedLines.replace(/\n{2,}$/g, "\n");
