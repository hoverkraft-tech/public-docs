...
**Solution 3: Clean dependencies (inside the tooling container)**

Open the repository in the devcontainer or enter it via your wrapper (`make shell`) and then run:

```bash
cd application
rm -rf node_modules package-lock.json   # or .venv, vendor/, etc.
npm ci
npm run build
```
